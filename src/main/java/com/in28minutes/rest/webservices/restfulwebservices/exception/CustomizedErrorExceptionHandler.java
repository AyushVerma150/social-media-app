...
public class CustomizedErrorExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public final @NotNull ResponseEntity<ErrorDetails> handleAllExceptions(@NotNull Exception ex, WebRequest request) {
        ErrorDetails errorDetails = new ErrorDetails( LocalDate.now( ), ex.getMessage( ), request.getDescription( false ) );
        return new ResponseEntity<ErrorDetails>( errorDetails, HttpStatus.INTERNAL_SERVER_ERROR );
    }

    @ExceptionHandler(UserNotFoundException.class)
    public final @NotNull ResponseEntity<ErrorDetails> handleNotFoundException(@NotNull Exception ex, WebRequest request) {
        ErrorDetails errorDetails = new ErrorDetails( LocalDate.now( ), ex.getMessage( ), request.getDescription( false ) );
